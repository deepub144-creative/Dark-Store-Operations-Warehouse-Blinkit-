...
export default function AssistantStoreManagerApp() {
  ...
  const [activeTab, setActiveTab] = useState("grn"); // grn | putaway | cycle | dad | handover
  ...
  const [dadPhoto, setDadPhoto] = useState("https://placehold.co/150x150/1e293b/fff?text=Damage+Photo");
  ...
  function handleScan(code) {
    ...
    if (matchedSku) {
      ...
      // Default to first SKU if demo scan code
  ...
  async function approveGrn(grnId, skuId, addedQty) {
    try {
      ...
      // Update SKU Inventory Stock Level

...
export default function DeliveryCaptainApp() {
  ...
  // Camera stream for selfie
  const selfieVideoRef = useRef(null);

  // Start Selfie Camera Stream
  useEffect(() => {
  ...
  // Listen to Firestore Staged Unassigned Orders (LIVE SYNC FROM OD PICKER APP!)
  useEffect(() => {
  ...
  // Accept Staged Order for Delivery
  ...
  // Confirm Pickup via Order QR Code Scan
  ...
  // Complete Handover & Mark Delivered
  async function handleCompleteHandover() {
    if (!activeOrder) return;
    const totalDeliverySec = elapsedSec;
    const isSlaMet = totalDeliverySec <= 480; // 8 min target
  ...
  return (
    <div style={S.wrap}>
      ...
      <main style={S.main}>
        {/* STEP 1: ATTENDANCE & GEOFENCE CHECK-IN */}
        {activeStep === "checkin" && (
          <div style={S.panel}>
        ...
        {/* STEP 2: STAGED ORDER PICKUP QUEUE */}
        {activeStep === "pickup" && (
          <div>
        ...
        {/* STEP 3: SCAN ORDER QR CODE FOR PICKUP */}
        ...
        {/* STEP 5: EN ROUTE LAST-MILE NAVIGATION */}
        ...
        {/* STEP 6: HANDOVER & PHOTO PROOF COMPLETION */}
        {activeStep === "handover" && (
          <div style={S.panel}>

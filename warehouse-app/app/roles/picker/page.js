...
export default function OdPickerApp() {
  ...
  const [activeOrder, setActiveOrder] = useState(null); // order currently being picked
  ...
  // Listen to Unassigned Pending Orders in Firestore
  useEffect(() => {
    if (!isOnline) {
      stopOrderAlarm();
      return;
  ...
  function handleItemScan(code) {
    ...
    if (matchedIndex !== -1) {
      ...
      // Auto pick first unpicked item as demo scan fallback
  ...
  // Staging - Ready for Delivery (CRITICAL HANDOFF TRIGGER FOR CAPTAIN APP)
  async function handleMarkReadyForDelivery() {
    if (!activeOrder) return;
    const finalSec = elapsedSec;
    const isSlaAchieved = finalSec <= 180; // 3 min SLA
  ...
  return (
    <div style={S.wrap}>
      ...
      <main style={S.main}>
          ...
          <div>
            {unassignedOrders.length > 0 ? (
              ...
              <div>
                ...
                {/* Rack Replenishment Task List during idle time */}
        ...
        {/* STEP 2: ROUTE NAVIGATION & PICKING */}
